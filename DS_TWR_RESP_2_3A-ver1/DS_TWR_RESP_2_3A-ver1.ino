...
/*Define the device*/
#define APP_NAME "DS TWR RESP_2 v1.0"
...
static uint64_t poll_rx_ts_2;
static uint64_t resp_tx_ts_2;
static uint64_t final_rx_ts_2;
...
void loop (){
  while (1){
    ...
    if (status_reg_2 & SYS_STATUS_RXFCG_BIT_MASK){
      ...
      if (frame_len <= RX_BUF_LEN_2){
        dwt_readrxdata(rx_buffer_2, frame_len, 0);
      }
      ...
      if (memcmp(rx_buffer_2, rx_poll_msg_2, ALL_MSG_COMMON_LEN_2) == 0){
        uint32_t resp_tx_time_2;

        /* Retrieve poll reception timestamp. */
        poll_rx_ts_2 = get_rx_timestamp_u64();

        /* Set send time for response. See NOTE 9 below. */
        resp_tx_time_2 = (poll_rx_ts_2 + (POLL_RX_TO_RESP_TX_DLY_UUS * UUS_TO_DWT_TIME)) >> 8;
        dwt_setdelayedtrxtime(resp_tx_time_2);
        ...
        tx_resp_msg_2[ALL_MSG_SN_IDX_2] = frame_seq_nb_2;
        ...
        if (status_reg_2 & SYS_STATUS_RXFCG_BIT_MASK) {
           ...
           */
          rx_buffer_2[ALL_MSG_SN_IDX_2] = 0;
          if (memcmp(rx_buffer_2, rx_final_msg_2, ALL_MSG_COMMON_LEN_2) == 0){
            uint32_t poll_tx_ts_2, resp_rx_ts_2, final_tx_ts_2;
            uint32_t poll_rx_ts_2, resp_tx_ts_2, final_rx_ts_2;
            double Ra_2, Rb_2, Da_2, Db_2;
            int64_t tof_dtu_2;

            /* Retrieve response transmission and final
             * reception timestamps. */
            resp_tx_ts_2 = get_tx_timestamp_u64();
            final_rx_ts_2 = get_rx_timestamp_u64();

            /* Get timestamps embedded in the final message. */
            final_msg_get_ts(&rx_buffer_2[FINAL_MSG_POLL_TX_TS_IDX_2], &poll_tx_ts_2);
            final_msg_get_ts(&rx_buffer_2[FINAL_MSG_RESP_RX_TS_IDX_2], &resp_rx_ts_2);
            final_msg_get_ts(&rx_buffer_2[FINAL_MSG_FINAL_TX_TS_IDX_2], &final_tx_ts_2);

            /* Compute time of flight. 32-bit subtractions give
             * correct answers even if clock has wrapped.
             * See NOTE 12 below. */
            poll_rx_ts_2 = (uint32_t)poll_rx_ts_2;
            resp_tx_ts_2 = (uint32_t)resp_tx_ts_2;
            final_rx_ts_2= (uint32_t)final_rx_ts_2;
            ...
            tof_dtu_2 = (int64_t)((Ra_2 * Rb_2 - Da_2 * Db_2) / (Ra_2 + Rb_2 + Da_2 + Db_2));

            tof_2 = tof_dtu_2 * DWT_TIME_UNITS;
            distance_2 = tof_2 * SPEED_OF_LIGHT;
            ...
            Serial.println("0 "+String(distance_2*100)+" 200"); 
